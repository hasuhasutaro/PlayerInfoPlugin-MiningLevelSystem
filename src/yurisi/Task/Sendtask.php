	...
	public function onRun($tick){
		foreach($this->Main->getServer()->getInstance()->getOnlinePlayers() as $player) {
			...
			if($this->Main->isOn($player)) {
				...
				$this->sendData($player,"§e所持金: ".FtMoneyAPI::getInstance() -> getMoney($player),1);
				$this->sendData($player,"§e現在の採掘レベル: ".MiningLevelAPI::getInstance() -> getLevel($player)."(次のレベルまで".MiningLevelAPI::getInstance() -> getLevelUpExp($player).")",2);
